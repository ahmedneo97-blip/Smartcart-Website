...
<body class="bg-gray-50 font-sans">
    ...
    document.getElementById('loginForm').addEventListener('submit', async (e) => {
        ...
        const resp = await fetch('api/login.php', { method: 'POST', body: form });
    ...
    document.getElementById('registerForm').addEventListener('submit', async (e) => {
        e.preventDefault();
        const form = new FormData(e.target);
        const resp = await fetch('api/register.php', { method: 'POST', body: form });
        const json = await resp.json();
        if (json.status === 'success') {
    ...
    document.getElementById('heroShop').addEventListener('click', () => {
        window.scrollTo({ top: document.querySelector('#productsGrid').offsetTop - 80, behavior: 'smooth' });
    });
